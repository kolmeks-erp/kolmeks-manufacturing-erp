    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-900 dark:text-white">Controlled Document Catalog</h2>
        </div>
              ...
              {loading ? (
                ...
                reportData.docsTable.map((d: any) => (
                  <tr key={d.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                    <td className="px-6 py-4 font-semibold text-slate-900 dark:text-white">{d.title || d.file_name}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{d.category || 'General'}</td>
                    <td className="px-6 py-4 text-mono font-bold text-blue-600 dark:text-blue-400">{d.version || 'v1.0'}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{d.expiration_date ? new Date(d.expiration_date).toLocaleDateString() : 'N/A'}</td>
