    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-900 dark:text-white">Employee Roster</h2>
        </div>
              ...
              {loading ? (
                ...
                reportData.employeesTable.map((e: any) => (
                  <tr key={e.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition">
                    <td className="px-6 py-4 font-mono font-bold text-indigo-600 dark:text-indigo-400">{e.employee_code || e.id.slice(0, 8)}</td>
                    <td className="px-6 py-4 font-semibold text-slate-900 dark:text-white">
                      {`${e.first_name || ''} ${e.last_name || ''}`.trim() || e.name || 'Staff Member'}
                    </td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{e.department?.name || 'Operations'}</td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{e.job_title || 'Specialist'}</td>
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 bg-emerald-50 text-emerald-600 border border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20 rounded-full text-xs font-semibold uppercase">
                        {e.status || 'Active'}
                      </span>
                    </td>
                  </tr>
