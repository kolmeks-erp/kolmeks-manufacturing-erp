        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${filterStatus === '' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-900 border border-slate-200/60'}`}
            >
              All Entries ({entries.length})
            </button>
            <button
              onClick={() => setFilterStatus('DRAFT')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${filterStatus === 'DRAFT' ? 'bg-amber-600 text-white' : 'bg-slate-100 text-amber-700 hover:bg-amber-50 border border-amber-200'}`}
            >
              Drafts
            </button>
            <button
              onClick={() => setFilterStatus('POSTED')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${filterStatus === 'POSTED' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-emerald-700 hover:bg-emerald-50 border border-emerald-200'}`}
            >
              Posted
            </button>
            <button
              onClick={() => setFilterStatus('VOIDED')}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${filterStatus === 'VOIDED' ? 'bg-rose-600 text-white' : 'bg-slate-100 text-rose-700 hover:bg-rose-50 border border-rose-200'}`}
            >
              Voided / Reversed
            </button>
          </div>
          ...
          <form onSubmit={handleSearch} className="flex flex-wrap items-center gap-2">
            <input
              ...
              className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-900 text-xs focus:outline-none focus:border-emerald-500"
            />
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-1.5 bg-white border border-slate-200 rounded-lg text-slate-900 placeholder-slate-400 text-xs focus:outline-none focus:border-emerald-500"
              />
            </div>
            <button
              type="submit"
              className="px-3.5 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium rounded-lg border border-slate-200 transition-colors"
            >
              Filter
            </button>
          </form>
              ...
              {entries.length === 0 ? (
                ...
                entries.map(entry => (
                  ...
                  >
                    ...
                    <td className="p-3.5 text-slate-600">{entry.entry_date}</td>
                    <td className="p-3.5 text-slate-600 font-medium">
                      {entry.period?.period_name || '—'}
                    </td>
                    <td className="p-3.5 max-w-md truncate font-medium text-slate-900">
                      {entry.description}
                      {entry.reference_id && (
                        <span className="ml-2 text-[10px] text-slate-400 font-mono">[{entry.reference_type}: {entry.reference_id}]</span>
                      )}
                    </td>
