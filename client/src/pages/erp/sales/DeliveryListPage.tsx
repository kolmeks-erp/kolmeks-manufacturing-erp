...
export const DeliveryListPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    if (norm === 'DISPATCHED' || norm === 'IN_TRANSIT') {
      return (
        <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 flex items-center space-x-1 w-fit">
    ...
    return (
      <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-700 text-slate-300 border border-slate-600 flex items-center space-x-1 w-fit">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-800/80 backdrop-blur-sm p-6 rounded-2xl border border-slate-700/60 shadow-lg">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center space-x-3">
            ...
            <span>Delivery Orders & Shipment Tracking</span>
          </h1>
                ...
                deliveries.map((d) => (
                  <tr key={d.id} className="hover:bg-slate-700/30 transition">
                    <td className="px-6 py-4 font-bold text-white">
                      <Link to={`${ERP_BASE_PATH}/sales/deliveries/${d.id}`} className="hover:text-indigo-400 transition">
                        {d.delivery_number}
                      </Link>
                    </td>
                    <td className="px-6 py-4 font-medium text-white">{d.customer_master?.company_name || 'N/A'}</td>
                    <td className="px-6 py-4 text-slate-300">
                      <div>{d.carrier || 'Unassigned Carrier'}</div>
                      <div className="text-xs text-indigo-400 font-mono">{d.tracking_reference || 'No Tracking Ref'}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <Link
                          ...
                          className="p-2 bg-slate-700/60 hover:bg-indigo-600 text-slate-300 hover:text-white rounded-lg transition"
                          title="View Details"
                        >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl p-6 space-y-5">
            ...
            <form onSubmit={handleCreateDelivery} className="space-y-4">
              ...
              <div>
                <label className="block text-xs uppercase font-semibold text-slate-300 mb-1">Carrier & Tracking</label>
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-700 text-white rounded-xl p-2.5 text-sm focus:outline-none focus:border-indigo-500"
                  />
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs uppercase font-semibold text-slate-300 mb-1">Shipping Destination Address</label>
                <textarea
                  rows={2}
      ...
      {showDispatchModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-5">
            <h3 className="text-lg font-bold text-white">Dispatch Shipment</h3>
            <form onSubmit={handleDispatch} className="space-y-4">
              <div>
                <label className="block text-xs uppercase font-semibold text-slate-300 mb-1">Carrier Name</label>
                <input
                  type="text"
                  value={carrier}
                  onChange={(e) => setCarrier(e.target.value)}
              ...
              <div>
                <label className="block text-xs uppercase font-semibold text-slate-300 mb-1">Tracking Number</label>
                <input
                  type="text"
                  value={trackingRef}
                  onChange={(e) => setTrackingRef(e.target.value)}
              ...
              <div className="flex justify-end space-x-3 pt-3 border-t border-slate-700">
                <button
                  ...
                  className="px-4 py-2 bg-slate-700 text-white text-sm font-medium rounded-xl"
                >
                  Cancel
                </button>
                <button
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-md shadow-2xl p-6 space-y-5">
            <h3 className="text-lg font-bold text-white">Confirm Customer Delivery</h3>
            <form onSubmit={handleConfirmDelivery} className="space-y-4">
              <div>
                <label className="block text-xs uppercase font-semibold text-slate-300 mb-1">Proof of Delivery / Receiver Name</label>
                <input
                  type="text"
              ...
              <div className="flex justify-end space-x-3 pt-3 border-t border-slate-700">
                ...
                <button
                  type="submit"
                  disabled={actionLoading}
                  className="px-4 py-2 bg-emerald-600 text-white text-sm font-medium rounded-xl"
                >
                  Mark DELIVERED
                </button>
              </div>
