...
export const ProductListPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn w-full text-slate-800 dark:text-slate-100">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Product Master Directory</h1>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
              Manage engineered products, sub-assemblies, and manufactured components used throughout the Kolmeks ERP.
            </p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#0F2647] p-4 rounded-2xl shadow-xs border border-slate-200 dark:border-slate-800 space-y-3">
        <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <select
              ...
              className="px-3 py-2 text-xs bg-slate-50 dark:bg-[#071220] border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium cursor-pointer"
            >
            ...
            {(selectedCategory || selectedType || selectedStatus || debouncedSearch) && (
              <button
                ...
                className="px-3 py-2 text-xs font-bold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-xl transition-colors flex items-center gap-1 cursor-pointer"
              >
            ...
            <button
              ...
              className="p-2 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-700 rounded-xl transition-colors cursor-pointer"
              title="Refresh List"
            >
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-[#0F2647] rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 dark:bg-[#0B1E36] text-slate-500 dark:text-slate-400 uppercase font-mono text-[10px] tracking-wider border-b border-slate-100 dark:border-slate-800">
                <tr>
                  <th className="py-3.5 px-5 whitespace-nowrap font-bold">Code</th>
                  <th className="py-3.5 px-5 whitespace-nowrap font-bold">Product Name</th>
                  <th className="py-3.5 px-5 whitespace-nowrap font-bold">Category</th>
                  <th className="py-3.5 px-5 whitespace-nowrap font-bold">Type</th>
                  <th className="py-3.5 px-5 whitespace-nowrap font-bold">Material</th>
                  <th className="py-3.5 px-5 whitespace-nowrap font-bold">Unit</th>
                  <th className="py-3.5 px-5 whitespace-nowrap font-bold">Revision</th>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-sans text-slate-800 dark:text-slate-200">
                {products.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50/80 dark:hover:bg-[#163761]/50 transition-colors">
                    <td className="py-3.5 px-5 font-mono font-bold text-blue-700 dark:text-blue-400 whitespace-nowrap">
                      <span className="inline-block bg-blue-50 dark:bg-blue-950/50 px-2.5 py-1 rounded-lg border border-blue-100 dark:border-blue-800 whitespace-nowrap">
                        {row.product_code}
                      </span>
                    </td>
                    <td className="py-3.5 px-5 whitespace-nowrap">
                      ...
                      {row.part_number && (
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 font-mono">P/N: {row.part_number}</div>
                      )}
                    </td>
                    <td className="py-3.5 px-5 text-slate-700 dark:text-slate-300 whitespace-nowrap">
                      {row.category?.name || <span className="text-slate-400 dark:text-slate-500 italic">Unassigned</span>}
                    </td>
                    <td className="py-3.5 px-5 whitespace-nowrap">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-lg text-[11px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 whitespace-nowrap">
                        {formatTypeLabel(row.product_type)}
                      </span>
                    </td>
                    <td className="py-3.5 px-5 text-slate-600 dark:text-slate-400 whitespace-nowrap">{row.material || '—'}</td>
                    <td className="py-3.5 px-5 uppercase font-mono font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{row.unit}</td>
                    <td className="py-3.5 px-5 font-mono font-bold text-slate-700 dark:text-slate-300 whitespace-nowrap">{row.revision || 'R0'}</td>
                    <td className="py-3.5 px-5 whitespace-nowrap">
                    ...
                    <td className="py-3.5 px-5 text-right whitespace-nowrap space-x-1">
                      <button
                        ...
                        className="inline-flex items-center gap-1 p-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg transition-colors cursor-pointer"
                        title="View Profile"
                      >
          ...
          {/* PAGINATION BAR */}
          {totalRecords > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-[#0B1E36] text-xs text-slate-600 dark:text-slate-400">
              <div>
                Showing <span className="font-semibold text-slate-900 dark:text-white">{(page - 1) * limit + 1}</span> to{' '}
                <span className="font-semibold text-slate-900 dark:text-white">{Math.min(page * limit, totalRecords)}</span> of{' '}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 border border-slate-300 dark:border-slate-700 rounded-lg font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors cursor-pointer"
                >
                  Previous
                </button>
