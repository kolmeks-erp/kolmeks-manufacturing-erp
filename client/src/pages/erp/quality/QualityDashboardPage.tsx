  ...
  return (
    <div className="space-y-6">
      <ERPPageHeader
        ...
        subtitle="Monitor incoming materials, production inspection results, quality holds, and Non-Conformance Reports (NCR)."
        actions={
          <div className="flex items-center gap-3">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 shadow-sm transition-colors"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-xs text-slate-500 mt-3 flex items-center gap-1">
            <span className="font-semibold text-slate-700">{kpis?.totalInspections || 0}</span> Total Inspections Recorded
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Quality Holds</p>
              <h3 className="text-3xl font-extrabold text-slate-900 mt-1">{kpis?.activeHolds || 0}</h3>
            </div>
          ...
          <p className="text-xs text-slate-500 mt-3">
            Quarantined material awaiting release decision
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-xs text-slate-500 mt-3 flex items-center gap-1">
            <span className="font-bold text-rose-600">{kpis?.overdueNCRs || 0}</span> Overdue Action Items
          </p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
          ...
          <div className="space-y-3">
            {recentNCRs.length === 0 ? (
              ...
              recentNCRs.map((ncr) => (
                <div key={ncr.id} className="p-3 bg-slate-50 rounded-lg border border-slate-200 space-y-1.5 hover:border-slate-300 transition-colors">
                  <div className="flex items-center justify-between">
                    <Link to={`/secure-kolmeks-x0y0/quality/ncr/${ncr.id}`} className="text-xs font-bold text-blue-600 hover:underline">
                      {ncr.ncr_number}
                    </Link>
