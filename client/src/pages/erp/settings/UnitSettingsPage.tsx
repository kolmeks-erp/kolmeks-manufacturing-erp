...
import { UnitItem } from '../../../types/settings';
import { Scale, Plus, Edit2, CheckCircle2, AlertCircle, X } from 'lucide-react';

export const UnitSettingsPage: React.FC = () => {
  const [units, setUnits] = useState<UnitItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<Partial<UnitItem> | null>(null);
  ...
  const handleOpenModal = (item?: UnitItem) => {
    setEditingItem(
      item || {
        code: '',
        name: '',
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await settingsService.saveUnit(editingItem);
      setToast({ message: 'Unit of measure saved.', type: 'success' });
      setModalOpen(false);
      fetchUnits();
    } catch (err: any) {
      setToast({ message: err.message || 'Failed to save UOM.', type: 'error' });
    } finally {
      setSaving(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Define system measurement standards for manufacturing, stock items, weight, and volume.
          </p>
        </div>
        <button
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-700/50 text-gray-500 dark:text-gray-400 text-xs font-semibold uppercase tracking-wider">
                ...
                <th className="py-3 px-4">Conversion Factor</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700 text-sm">
              {units.map((item) => (
                <tr key={item.code} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="py-3 px-4 text-gray-600 dark:text-gray-300">{item.category}</td>
                  <td className="py-3 px-4 font-bold text-gray-800 dark:text-gray-200">{item.symbol}</td>
                  <td className="py-3 px-4 font-mono text-gray-700 dark:text-gray-300">{item.conversion_factor}</td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 max-w-md w-full p-6 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-gray-100 dark:border-gray-700">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">
                {editingItem.id ? 'Edit Unit' : 'New Unit of Measure'}
              </h3>
            ...
            <form onSubmit={handleSave} className="space-y-4 text-sm">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={editingItem.category || 'Quantity'}
              ...
              <div>
                <label className="block font-semibold text-gray-700 dark:text-gray-300 mb-1">Unit Name *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block font-semibold text-gray-700 dark:text-gray-300 mb-1">Conversion Factor</label>
                  <input
                    type="number"
                    step="0.000001"
                    value={editingItem.conversion_factor || 1.0}
                    onChange={(e) => setEditingItem({ ...editingItem, conversion_factor: parseFloat(e.target.value) || 1.0 })}
                    className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg font-mono text-gray-900 dark:text-white"
                  />
                </div>
              </div>
