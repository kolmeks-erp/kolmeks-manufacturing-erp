      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-950 border border-slate-800 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:border-purple-500"
          />
        </div>
      </div>
                ...
                variances.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-slate-800/40 transition">
                      <td className="p-4 font-semibold text-white">{v.variance_number}</td>
                      <td className="p-4 font-mono text-purple-400 text-xs">
                        {v.production_order?.production_order_number}
                      </td>
                      <td className="p-4">
                        <div className="text-slate-200 font-medium">{v.product?.name}</div>
                        <div className="text-xs text-slate-500">{v.product?.product_code}</div>
                      </td>
                      <td className={`p-4 font-medium ${(v.material_qty_variance || 0) > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                        {formatCurrency(v.material_qty_variance)}
                      </td>
                      <td className={`p-4 font-medium ${(v.labor_efficiency_variance || 0) > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                        {formatCurrency(v.labor_efficiency_variance)}
                      </td>
                      <td className={`p-4 font-medium ${(v.overhead_variance || 0) > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                        {formatCurrency(v.overhead_variance)}
                      </td>
                      <td className={`p-4 font-bold ${totalVar > 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
