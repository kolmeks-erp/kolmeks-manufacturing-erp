    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-xl transition shadow-lg shadow-blue-500/20"
          >
          ...
          {costing && costing.status !== 'POSTED' && (
            <button
              ...
              disabled={posting}
              className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold rounded-xl transition shadow-lg shadow-emerald-500/20"
            >
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Product: <span className="text-white font-medium">{order?.product?.name}</span> ({order?.product?.product_code})
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-slate-950 p-4 rounded-xl border border-slate-800">
          <div>
            <div className="text-xs text-slate-500 font-medium">Planned Qty</div>
            <div className="text-white font-bold">{order?.planned_quantity} {order?.product?.unit}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500 font-medium">Completed Qty</div>
            <div className="text-emerald-400 font-bold">{order?.completed_quantity || 0} {order?.product?.unit}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500 font-medium">Total Cost</div>
            <div className="text-white font-bold">{formatCurrency(costing?.total_cost)}</div>
          </div>
          <div>
            <div className="text-xs text-slate-500 font-medium">Unit Cost</div>
            <div className="text-emerald-400 font-bold">{formatCurrency(costing?.unit_cost)}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          <div className="text-xs font-semibold text-slate-400 uppercase">Direct Material Cost</div>
          <div className="text-xl font-bold text-white mt-1">{formatCurrency(costing?.material_cost)}</div>
          <div className="text-xs text-slate-500 mt-1">BOM Planned: {formatCurrency(costing?.planned_material_cost)}</div>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          <div className="text-xs font-semibold text-slate-400 uppercase">Direct Labor Cost</div>
          <div className="text-xl font-bold text-white mt-1">{formatCurrency(costing?.labor_cost)}</div>
          <div className="text-xs text-slate-500 mt-1">Routing Planned: {formatCurrency(costing?.planned_labor_cost)}</div>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          <div className="text-xs font-semibold text-slate-400 uppercase">Manufacturing Overhead</div>
          <div className="text-xl font-bold text-white mt-1">{formatCurrency(costing?.overhead_cost)}</div>
          <div className="text-xs text-slate-500 mt-1">Config Rate: {formatCurrency(costing?.planned_overhead_cost)}</div>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
          <div className="text-xs font-semibold text-slate-400 uppercase">Cost Variance</div>
          <div className={`text-xl font-bold mt-1 ${(costing?.total_variance || 0) >= 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
            {formatCurrency(costing?.total_variance)}
          </div>
          <div className="text-xs text-slate-500 mt-1">Variance %: {costing?.total_variance_pct || 0}%</div>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6">
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-blue-400 uppercase tracking-wider">1. Direct Material Consumptions</h4>
            ...
            <table className="w-full text-left text-xs text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/50">
                {matComponents.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-4 text-center text-slate-500">No material consumptions recorded yet.</td>
                  </tr>
                ) : (
                  matComponents.map((c, idx) => (
                    <tr key={idx}>
                      <td className="p-3 font-medium text-white">{c.description}</td>
                      <td className="p-3">{c.planned_quantity} {c.unit_of_measure}</td>
                      <td className="p-3 font-semibold text-blue-400">{c.actual_quantity} {c.unit_of_measure}</td>
                      <td className="p-3">{formatCurrency(c.unit_rate)}</td>
                      <td className="p-3 font-semibold text-white">{formatCurrency(c.actual_cost)}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-emerald-400 uppercase tracking-wider">2. Direct Labor Foundation</h4>
          <div className="border border-slate-800 rounded-xl overflow-hidden">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950 text-slate-400 font-semibold border-b border-slate-800">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800/50">
                {labComponents.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-4 text-center text-slate-500">No labor operations recorded yet.</td>
                  </tr>
                ) : (
                  labComponents.map((c, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="p-3 font-semibold text-emerald-400">{c.actual_quantity} hrs</td>
        ...
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-purple-400 uppercase tracking-wider">3. Manufacturing Overhead Allocation</h4>
          <div className="border border-slate-800 rounded-xl overflow-hidden">
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="bg-slate-950 text-slate-400 font-semibold border-b border-slate-800">
                <tr>
              ...
              <tbody className="divide-y divide-slate-800/50">
                {ovhComponents.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-4 text-center text-slate-500">No overhead allocation calculated yet.</td>
                  </tr>
                ) : (
                  ovhComponents.map((c, idx) => (
                    <tr key={idx}>
                      <td className="p-3 font-medium text-white">{c.description}</td>
                      <td className="p-3">{c.planned_quantity} hrs</td>
                      <td className="p-3 font-semibold text-purple-400">{c.actual_quantity} hrs</td>
                      <td className="p-3">{formatCurrency(c.unit_rate)}/hr</td>
                      <td className="p-3 font-semibold text-white">{formatCurrency(c.actual_cost)}</td>
                    </tr>
                  ))
