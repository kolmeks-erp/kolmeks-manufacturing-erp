  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-4 bg-slate-900 border border-slate-800 p-4 rounded-xl">
        <label className="text-sm font-medium text-slate-400">Report View:</label>
        <select
          value={reportType}
              ...
              {loading ? (
                ...
                reports.map((r, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="p-4 text-slate-300">{formatCurrency(r.material_cost)}</td>
                    <td className="p-4 text-slate-300">{formatCurrency(r.labor_cost)}</td>
                    <td className="p-4 text-slate-300">{formatCurrency(r.overhead_cost)}</td>
                    <td className="p-4 text-white font-bold">{formatCurrency(r.total_cost)}</td>
                    <td className="p-4 text-emerald-400 font-semibold">{formatCurrency(r.average_unit_cost || r.unit_cost)}</td>
                  </tr>
                ))
              )}
            </tbody>
