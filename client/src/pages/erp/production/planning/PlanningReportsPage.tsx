        ...
        <div className="bg-white dark:bg-[#0F2647] border border-slate-200 dark:border-slate-800/80 rounded-xl overflow-hidden shadow-xs">
          <div className="p-4 bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-200 flex items-center gap-2">
              <FileSpreadsheet className="text-purple-600 dark:text-purple-400" size={16} /> Plan vs Actual Execution Summary ({reportData.length} lines)
            </h3>
          </div>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
                {reportData.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-4 py-3 font-mono font-semibold text-blue-600 dark:text-blue-400">{row.plan_number}</td>
                    <td className="px-4 py-3">
                      <div className="font-semibold text-slate-900 dark:text-slate-100">{row.product_name}</div>
                      <div className="font-mono text-slate-500 dark:text-slate-400">{row.product_code}</div>
                    </td>
                    <td className="px-4 py-3 text-right font-mono font-bold text-slate-900 dark:text-slate-200">
                      {row.planned_quantity} {row.unit}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-slate-700 dark:text-slate-300">{row.scheduled_quantity}</td>
                    <td className="px-4 py-3 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400">{row.completed_quantity}</td>
                    <td className="px-4 py-3 text-right font-mono text-amber-600 dark:text-amber-400">{row.remaining_quantity}</td>
                    <td className="px-4 py-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <span className="font-mono font-bold">{row.completion_pct}%</span>
                        <div className="w-16 h-1.5 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-200 dark:border-slate-800 hidden md:block">
                          <div
                    ...
                    <td className="px-4 py-3 font-mono text-slate-700 dark:text-slate-300">{row.required_date}</td>
