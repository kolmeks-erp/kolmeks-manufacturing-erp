                ...
                {(['MONTH', 'WEEK', 'DAY'] as const).map((mode) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md font-medium transition-colors ${
                      viewMode === mode ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    {mode}
                  </button>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 bg-slate-950 border border-slate-800 hover:bg-slate-800 text-slate-300 rounded-lg transition-colors"
            >
              <ChevronLeft size={16} />
            </button>
        ...
        {loading ? (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4 shadow-sm">
            <h3 className="text-base font-semibold text-slate-100 flex items-center gap-2 mb-2">
              <CalendarIcon className="text-amber-400" size={18} /> Scheduled Jobs Timeline ({events.length})
            </h3>
            ...
            <div className="space-y-3">
              {events.map((ev) => (
                <div
                  key={ev.id}
                  className="bg-slate-950 border border-slate-800 rounded-xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 hover:border-slate-700 transition-colors"
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono font-bold text-blue-400">{ev.title}</span>
                  ...
                  <div className="text-xs font-mono text-slate-300 bg-slate-900 px-3 py-2 rounded-lg border border-slate-800 flex items-center gap-2">
