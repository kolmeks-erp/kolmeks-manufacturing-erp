              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
                {schedules.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-4 py-3 font-mono font-semibold text-blue-600 dark:text-blue-400">{s.schedule_number}</td>
                    <td className="px-4 py-3">
                      <div className="font-semibold text-slate-900 dark:text-slate-100">
                        {s.production_order?.production_order_number || 'PO'}
                      </div>
                      <div className="text-slate-500 dark:text-slate-400">{s.production_order?.product?.name}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="font-medium text-slate-800 dark:text-slate-200">{s.work_center?.name}</span>
                      <span className="font-mono text-slate-500 dark:text-slate-400 block text-[11px]">{s.work_center?.code}</span>
                    </td>
                    <td className="px-4 py-3">
                      {s.machine ? (
                        <span className="font-mono text-indigo-600 dark:text-indigo-400">{s.machine.name}</span>
                      ) : (
                    ...
                    <td className="px-4 py-3 font-mono text-[11px]">
                      ...
                      <div className="text-slate-500 dark:text-slate-400">to {new Date(s.planned_end).toLocaleString()}</div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded border border-slate-200 dark:border-slate-700 flex items-center gap-1 transition-colors mx-auto"
                      >
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 max-w-lg w-full space-y-4 shadow-2xl">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              <Clock className="text-blue-600 dark:text-blue-400" size={20} /> Schedule Work Order Operation
            </h3>

            {conflictWarning && (
              <div className="p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-amber-800 dark:text-amber-400 rounded-lg text-xs font-medium flex items-center gap-2">
            ...
            <form onSubmit={handleCreateScheduleSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-700 dark:text-slate-300 mb-1">Production Order *</label>
                <select
                  required
                  value={poId}
                  onChange={(e) => setPoId(e.target.value)}
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded p-2 text-slate-900 dark:text-slate-200"
                >
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 mb-1">Work Center *</label>
                  <select
                    required
                    value={wcId}
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 mb-1">Machine Asset</label>
                  <select
                    value={machineId}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 mb-1">Planned Start Datetime *</label>
                  <input
                    type="datetime-local"
                    required
                    value={pStart}
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 mb-1">Planned End Datetime *</label>
                  <input
                    type="datetime-local"
                    required
                    value={pEnd}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 mb-1">Setup Time (Hours)</label>
                  <input
                    type="number"
                    step="0.5"
                    value={setupHrs}
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 mb-1">Run Time (Hours)</label>
                  <input
                    type="number"
                    step="0.5"
                    value={runHrs}
      ...
      {showRescheduleModal && selectedSchedule && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
              <Edit2 className="text-amber-600 dark:text-amber-400" size={20} /> Reschedule {selectedSchedule.schedule_number}
            </h3>

            <form onSubmit={handleRescheduleSubmit} className="space-y-4 text-xs">
              <div>
                <label className="block text-slate-700 dark:text-slate-300 mb-1">New Start Time</label>
                <input
                  type="datetime-local"
                  required
                  value={reschedStart}
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 mb-1">New End Time</label>
                <input
                  type="datetime-local"
                  required
                  value={reschedEnd}
              ...
              <div>
                <label className="block text-slate-700 dark:text-slate-300 mb-1">Reason for Rescheduling</label>
                <textarea
                  rows={2}
                  required
