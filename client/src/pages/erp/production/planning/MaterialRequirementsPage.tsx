...
const MaterialRequirementsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {totalShortages > 0 && (
        <div className="p-4 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl flex items-center justify-between text-amber-800 dark:text-amber-400 text-sm">
          ...
          <span className="text-xs bg-amber-200 dark:bg-amber-500/20 text-amber-900 dark:text-amber-300 px-2.5 py-1 rounded font-mono">
            Action Required
          </span>
        </div>
      )}
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
                {filteredMaterials.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-4 py-3 font-mono text-blue-600 dark:text-blue-400 font-semibold">{item.component_code}</td>
                    <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-100">{item.component_name}</td>
                    <td className="px-4 py-3 text-right font-mono font-bold text-slate-900 dark:text-slate-100">
                      {item.gross_requirement} {item.unit}
                    </td>
                    <td className="px-4 py-3 text-right font-mono text-slate-600 dark:text-slate-300">{item.on_hand_quantity}</td>
                    <td className="px-4 py-3 text-right font-mono text-slate-500 dark:text-slate-400">{item.reserved_quantity}</td>
                    <td className="px-4 py-3 text-right font-mono font-bold text-emerald-600 dark:text-emerald-400">{item.available_quantity}</td>
                    <td className="px-4 py-3 text-right font-mono font-bold">
                      {item.shortage_quantity > 0 ? (
                        <span className="text-rose-600 dark:text-rose-400">-{item.shortage_quantity}</span>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-center">
                      {item.status === 'AVAILABLE' && (
                        <span className="px-2.5 py-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 rounded-full font-semibold text-[11px] inline-flex items-center gap-1">
                          <CheckCircle2 size={12} /> AVAILABLE
                        </span>
                      )}
                      {item.status === 'PARTIAL' && (
                        <span className="px-2.5 py-1 bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-500/20 rounded-full font-semibold text-[11px] inline-flex items-center gap-1">
                          <AlertTriangle size={12} /> PARTIAL
                        </span>
                      )}
                      {item.status === 'SHORTAGE' && (
                        <span className="px-2.5 py-1 bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20 rounded-full font-semibold text-[11px] inline-flex items-center gap-1">
                          <AlertTriangle size={12} /> SHORTAGE
                        </span>
                      )}
                      {item.status === 'UNKNOWN' && (
                        <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-full font-semibold text-[11px] inline-flex items-center gap-1">
