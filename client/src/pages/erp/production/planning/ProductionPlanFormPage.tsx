        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
            <h3 className="text-base font-semibold text-slate-100 flex items-center gap-2">
              <ClipboardList className="text-blue-400" size={18} /> Plan Header Details
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Period Horizon</label>
                <select
                  value={periodType}
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-semibold text-slate-100 flex items-center gap-2">
                <Layers className="text-emerald-400" size={18} /> Planned Production Lines
              </h3>
              <button
                type="button"
                onClick={addLine}
                className="px-3 py-1.5 bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 text-xs font-medium rounded-lg border border-blue-500/20 flex items-center gap-1 transition-colors"
              >
            ...
            <div className="space-y-3">
              {lines.map((line, index) => (
                <div key={index} className="bg-slate-950 border border-slate-800 rounded-lg p-4 grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
                  <div className="md:col-span-4">
                    <label className="block text-[11px] text-slate-400 mb-1">Product</label>
                    <select
                      value={line.product_id}
                      onChange={(e) => updateLine(index, 'product_id', e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 text-slate-200 text-xs rounded px-2.5 py-1.5 focus:outline-none focus:border-blue-500"
                    >
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[11px] text-slate-400 mb-1">Planned Qty</label>
                    <input
                      type="number"
                      min="1"
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[11px] text-slate-400 mb-1">Required Date</label>
                    <input
                      type="date"
                  ...
                  <div className="md:col-span-2 flex items-center justify-end">
                    <button
                      ...
                      className="p-2 text-slate-400 hover:text-rose-400 disabled:opacity-30 transition-colors"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
