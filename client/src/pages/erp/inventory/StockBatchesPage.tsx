    ...
    <div className="p-6 space-y-6 bg-slate-900 min-h-screen text-slate-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-800/80 p-6 rounded-2xl border border-slate-700/60">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white tracking-tight">Batch & Lot Tracking Register</h1>
        ...
        <button
          ...
          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-medium transition flex items-center gap-2 text-sm shadow-lg shadow-indigo-600/20"
        >
          ...
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-slate-700/40">
              {loading ? (
                ...
                batches.map((batch) => (
                  <tr key={batch.id} className="hover:bg-slate-700/30 transition">
                    <td className="py-3.5 px-4 font-mono text-xs font-bold text-indigo-400">
                      {batch.batch_number}
                    </td>

                    <td className="py-3.5 px-4">
                      <div className="font-semibold text-white">{batch.products?.name || 'Component'}</div>
                      <div className="text-xs text-slate-400 font-mono">{batch.products?.product_code}</div>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right font-bold text-white">
                      {batch.quantity} <span className="text-xs text-slate-400 font-normal">{batch.products?.unit || 'pcs'}</span>
                    </td>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-white">Register Stock Batch / Lot</h3>
              </div>
            ...
            <form onSubmit={handleCreateBatch} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">Batch Status</label>
                  <select
                    value={status}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-medium shadow-lg shadow-indigo-600/20"
                >
