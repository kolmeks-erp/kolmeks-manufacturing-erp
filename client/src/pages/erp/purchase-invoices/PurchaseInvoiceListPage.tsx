...
export const PurchaseInvoiceListPage: React.FC = () => {
  ...
  const getMatchBadge = (match: MatchStatus) => {
    switch (match) {
      case 'MATCHED':
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-emerald-100 text-emerald-800">3-Way Matched</span>;
      case 'QUANTITY_VARIANCE':
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-amber-100 text-amber-800">Qty Variance</span>;
      case 'PRICE_VARIANCE':
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-orange-100 text-orange-800">Price Variance</span>;
      case 'BOTH_VARIANCE':
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-rose-100 text-rose-800">Qty & Price Variance</span>;
      case 'APPROVED_EXCEPTION':
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-blue-100 text-blue-800">Approved Exception</span>;
      default:
        return <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-slate-100 text-slate-700">{match}</span>;
    }
  };
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium text-slate-700 focus:ring-2 focus:ring-indigo-500"
          >
